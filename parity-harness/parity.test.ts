...
describe("Phase 2 parity harness", () => {
  it("keeps the legacy full-mix project fixture stable through v1 serde", () => {
    ...
    expect(roundTrip.path.ranged_constraints).toEqual(project.path.ranged_constraints);
  });
  ...
  it("keeps desktop path constraint ordinals stable through native path serde", () => {
    ...
    const roundTripPath = deserializePath(
      serializedPath,
      () => project.config.kinematic_constraints.default_intermediate_handoff_radius_meters
    );

    expect(roundTripPath.ranged_constraints).toEqual(project.path.ranged_constraints);

...
async function resource(request: Request): Promise<Response> {
  try {
    ...
    for (const record of await completed()) {
      // These caches contain integrity-verified release bytes. A hosting CORS
      // header (Vary: Origin) does not select different bytes, but module requests
      // include Origin while install fetches do not. Keep exact URLs and response
      // headers while allowing either request to retrieve the verified asset.
      const response = await (
        await caches.open(record.cache)
      ).match(request, {
        ignoreVary: true,
      });
